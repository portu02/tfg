...
?>
        ...
        <script>
                ...
                Email.send({
                    ...
                    Body: "<html><head><style>*{color: white;text-align:center;}span{color:red;} #richi {font-family: 'Times New Roman', serif;font-weight: bold;font-size: 40px;color: red;}div {position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);background-color: black;}</style></head><body><div class='contenedor'><h1><span id='richi'>RICHILICULAS</span></h1><hr style='background-color: white;'><h1>Compra de entradas</h1><h2><span>Pelicula:</span> <?= $nombre_pelicula ?? '' ?> <span>Sala:</span> <?= $id_sala ?? '' ?></h2><h2><?= $string_fila_columna ?? '' ?></h2><h2><span>Total:</span> <?= $total ?? '' ?> €</h2><hr style='background-color: white;'><p style='font-size:20px' >Gracias por confiar en Richiliculas para comprar tu entrada de cine, esperamos que disfrutes de la pelicula</p></div></body></html>"
                }).then(
