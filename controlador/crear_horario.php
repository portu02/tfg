...
/* SI NO HAY PELICULAS, NI SALAS NO CREA NADA */
if (count($registroarray) != 0 && count($numsalas) != 0) {
    /* INSERTA UNA SEMANA */
    for ($iu = 0; $iu < 8; $iu++) {
            ...
            if ($numsalas != null) {
                foreach ($numsalas as $numsalasi => $ns) {
                    ...
                    while ($horatiempo <= 22) {
                        ...
                        /* SI HAY MAS SALAS QUE PELICULAS HAN DE REPETIRSE HORARIOS DE PELICULAS*/
                        $hora = horario($horatiempo);
                        $fecha = $fecha_actual->format('Y-m-d');

                        $resultValidacion = $objthorario->comprobarSiLaPeliculaSeRepite($hora, $fecha, $peliculaElegida);

                        if (!$resultValidacion || count($numsalas) > count($registroarray)) {
